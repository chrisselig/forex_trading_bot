...
def walk_forward_validate(all_data: dict, pair: str, train_year: int, test_year: int) -> dict:
    """Train on one year, test on the next."""
    train_results = run_grid_search(all_data, pair, year_filter=train_year)
    ...
    test_results = run_grid_search(all_data, pair, year_filter=test_year)
...
def generate_report(
    ...
    lines += [
        ...
        "",
        "Train on 2025 data, test on 2026 data.",
        "",
    ...
    lines += [
        ...
        "## Caveats",
        "",
        "1. **Small sample size**: ~46 events over 18 months. Bootstrap CIs help but",
        "   structural regime changes are not captured.",
        "",
...
def main():
    ...
    wf_results: dict[str, dict] = {}
    for pair in args.pairs:
        wf = walk_forward_validate(all_data, pair, train_year=2025, test_year=2026)
        wf_results[pair] = wf
