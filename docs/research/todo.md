...
- ~~**Harden event alias collision logic**~~ — **DONE**: All four measures implemented. (1) Startup validator `validate_alias_uniqueness()` (`src/forex_bot/calendar/parser.py`) runs in `EventParser.__init__` (wired at orchestrator startup) and raises `ValueError` if any alias/canonical name maps to multiple countries — hard fail, not a warning. (2) Match-time country enforcement: `EventParser` skips any target whose `country != event.country`, and `_matches_filters` enforces the country allow-list. (3) Country-embedded aliases for the collision-prone AU events (`AU Employment Change`, `AU CPI`, etc. in `config/events.yaml`); US events stay generic but are disambiguated by their `country: "USD"` field. (4) Global-uniqueness unit test in `tests/unit/test_alias_uniqueness.py` (plus validator positive/negative and match-time country tests). Builds on PR #41's substring→exact-equality (`==`) matching fix. Note: the original Canadian "Employment Change" collision case no longer exists — there is no CAD target in `events.yaml`.
