...
- **Move the bot off the laptop to dedicated always-on hardware** — The bot + IB Gateway currently run on the user's laptop, which must stay powered on 24/5. Goal: a cheap, low-power, always-on host so the laptop is free for other use. Options to evaluate:
