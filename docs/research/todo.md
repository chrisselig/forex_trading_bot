...
- **Explore AUDUSD + Australian events** — AUDUSD on US events showed E[P&L]=+4.8 but N=19 (too few trades) with CI spanning zero (see [07-mc-eurusd-audusd.md](07-mc-eurusd-audusd.md)). Revisit with: (1) add Australian events to download script (RBA Rate Decision, Australia CPI, Australia Employment — all released during Asian session ~0:30-3:30 UTC), (2) download Dukascopy AUDUSD data for AU events, (3) run MC analysis on AU events separately, (4) test whether wider distance params capture more trades on US events. **Scheduling note**: Australian events fire on Sunday night / Monday early AM in Canada (forex opens Sunday 5 PM ET). The bot's APScheduler and IBC auto-start (`0 5 * * 1-5` cron) may need updates to handle Sunday session starts for AU events. Also check IB Gateway Sunday reconnection behavior.
