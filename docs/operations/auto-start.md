...
1. Cron fires `scripts/start_tws_and_bot.sh` at 5:00 AM MT (7:00 AM ET) on weekdays
...
```bash
# Runs at 5:00 AM MT (7:00 AM ET) on weekdays
0 5 * * 1-5 /home/doopdeep/00_data_projects/forex_trading_bot/scripts/start_tws_and_bot.sh >> ~/ibc/logs/cron_$(date +\%Y\%m\%d).log 2>&1
```
